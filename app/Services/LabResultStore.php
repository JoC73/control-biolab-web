...
class LabResultStore
{
    ...
    public function saveFromOrder(array $order): array
    {
        $record = [
            ...
            'category_name' => count($order['exam_items'] ?? []) > 1 ? count($order['exam_items']).' examenes' : $order['category_name'],
            'category_title' => count($order['exam_items'] ?? []) > 1 ? 'Resultados consolidados' : $order['category_title'],
            'tests' => $order['tests'] ?? [],
    ...
    private function fromRow(object $row): array
    {
        return [
            ...
            'exam_items' => property_exists($row, 'exam_items') ? $this->decodeJson($row->exam_items) : [],
            'order_total' => property_exists($row, 'order_total') ? (float) ($row->order_total ?? 0) : null,
            'paid_amount' => property_exists($row, 'paid_amount') ? (float) ($row->paid_amount ?? 0) : null,
            'payment_status' => property_exists($row, 'payment_status') ? $row->payment_status : null,
            'order_status' => property_exists($row, 'order_status') ? $row->order_status : null,
    ...
    private function toDatabase(array $record, bool $includeId = true): array
    {
        $data = [
            ...
            'exam_items' => json_encode($record['exam_items'] ?? [], JSON_UNESCAPED_UNICODE),
            'order_total' => $record['order_total'] ?? null,
